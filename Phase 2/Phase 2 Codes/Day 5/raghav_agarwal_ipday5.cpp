...
Mat blobimage(Mat image1, int **a)
{
	...
	return image;
		...
		//lth = 50;
		//uth = 7
void linedetect(Mat image1,Mat image,int x,int y,int r)
{
	
	int i, j;
	for (i =-90; i <= 90; i++)
	{
		j = x*cos(i*pi / 180) + y*sin(i*pi / 180);
		if (image.at<uchar>(j+r, i+90)<255)
		image.at<uchar>(j+r, i+90)++;

	}

}
Mat linedetect1(Mat image1)
{
	int i, j;
	int r = sqrt(image1.rows*image1.rows + image1.cols*image1.cols);
	Mat image(2*r,181, CV_8UC1, Scalar(0));
	for (i = 0; i < image1.rows;i++)
	for (j = 0; j < image1.cols;j++)
	if (image1.at<uchar>(i, j) == 255)
		linedetect(image1, image, i, j,r);
	return image;
}
void retainline1(Mat image1, int r, int o,Mat image)
{
	int i, j;
	for (i = 0; i < image1.rows; i++)
	{
		j = (r - i*cos(o*pi / 180)) / sin(o*pi / 180);
		if ((j>0) && j<image1.cols)
			if(image.at<uchar>(i, j) == 255)
		image1.at<uchar>(i, j) = 255;
	}

}
Mat retainline(Mat image1,Mat image,int th)
{
	int i, j;
	int r = sqrt(image.rows*image.rows + image.cols*image.cols);
	Mat img(image.rows, image.cols, CV_8UC1,Scalar(0));
	for (i = 0; i < image1.rows; i++){
		for (j = 1; j < image1.cols; j++)
		{
			if (image1.at<uchar>(i, j)>=th)
				retainline1(img, i-r, j-90,image);
		}
	}
	return img;
}
int main(){
	...
	image1=imread("C:\\l.png", CV_LOAD_IMAGE_GRAYSCALE);
	...
	//cvtColor(image1,image1, CV_BGR2HLS_FULL);
	createTrackbar("threshold", "window",&th, 255);
	//createTrackbar("upper threshold", "video", &uth, 255);
	//imgra = detectcolor(image1,160,15,0,10,240,15);
	//imshow("My window", imgra);
	/*while (a != 27){
	...
	imgray=linedetect1(image1);
	imshow("My window", imgray);
	while (a != 27){
		imgra = retainline(imgray, image1, th);
		imshow("window", imgra);
		a = waitKey(33);
	}
	waitKey(0);
