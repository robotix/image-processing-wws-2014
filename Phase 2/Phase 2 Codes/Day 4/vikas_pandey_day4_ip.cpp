...
void tempclass(){
	viks<int, int> yo;
	yo.vik = 12;
	cout << yo.func<char>(33, 2) << endl << yo.func<int>(33, 2) << endl;
	cout << yo.vik;
...
void blob_detect(Mat m, int count, int x, int y, Mat counter){
	...
	do{
		...
		for (a = -1; a <= 1; a++)
			for (b = -1; b <= 1; b++)
			{
				if ((p.y + b)<m.rows&&(p.y + b)>0 && (p.x + a)<m.cols&&(p.x + a)>0)
					if (m.at<uchar>(p.y + b, p.x + a) == 255 && counter.at<uchar>(p.y + b, p.x + a) == 0)
					{
						p1.x = p.x + a;
						p1.y = p.y + b;
						counter.at<uchar>(p.y+b,p.x + a) = count;
						q.push(p1);
					}
			}
...
void main(){
	Mat image;
	image = imread("V:\\blob3.png", CV_LOAD_IMAGE_COLOR);
