...
void Make_Line(Mat,int, int);
Point2D Shape_Detection(Mat,int);
Mat Color_Detection(Mat,int);
Point2D Obj_Center(Mat);

Point2D Lowest_Point(Mat img_load) // Enter Binary Img after color det.
{
	int i, j;
	Point2D end;
	for (i = 0; i < (img_load.rows); i++)
	for (j = 0; j < (img_load.cols); j++)
    	if (img_load.at<uchar>(i,j) == 255)
		{end.x = j; end.y = i;}
	
		return end;
}

Point2D Obj_Center(Mat img_load)   //Pass a binary img.
 {
	int i, j, s1 = 0,s2=0;

	for (i = 0; i < (img_load.cols); i++)
	for (j = 0; j < (img_load.rows); j++)
	   if(img_load.at<uchar>(j,i)==255)
		   s1++;
...
Mat Color_Detection(Mat img_load,int Code) //I created Bug.
{
	...
	int lower_h , upper_h , i, j, lower_s , upper_s , lower_l , upper_l ;

	if (Code == 0) // RED
	{
		lower_h = 220;  upper_h = 260;
		lower_s = 97;  upper_s = 225;
		lower_l = 66;  upper_l = 260;
	}
	else if (Code == 1) // BLUE
	{
		lower_h = 130;  upper_h = 210;
		lower_s = 72;  upper_s = 185;
		lower_l = 100;  upper_l = 260;
	}
	else if (Code == 2) // GREEN
	{
		lower_h = 30;  upper_h = 100;
		lower_s = 54;  upper_s = 154;
		lower_l = 150;  upper_l = 260;
	}
	else if (Code==3) // YELLOW
	{
		lower_h = 15;  upper_h = 52;
		lower_s = 60;  upper_s = 220;
		lower_l = 230;  upper_l = 260;
	}
   ...
   //Needed is white , BG is Black;
	return temp_img;
}  // Function end.

Point2D Shape_Detection(Mat img,int shape_side)
{
	int i;
	...
	for (i = 0; i < (c.size()); i++)
	{
		double error = (0.02)*(arcLength(c[i], true));
		approxPolyDP(c[i], app, error, true);
        if (app.size() == shape_side)
			return Cen;
	}
...
int main()
{
	...
	Mat img_out(img_load.rows, img_load.cols, CV_8UC1);
	img_out = Color_Detection(img_load, 0);

	imshow("Out2", img_out);
	Point2D C;
	C = Lowest_Point(img_out);

	Mat img_shape(img_load.rows, img_load.cols, CV_8UC1);
	C = Shape_Detection(img_out, 4);
...
int main()
	...
	*/
		...
		//Prog for Line Detection
/*
int main()
  {
	  int i, j;
	  Mat img;
	  img = imread("H://wallpapers//Line.png", CV_LOAD_IMAGE_GRAYSCALE);
